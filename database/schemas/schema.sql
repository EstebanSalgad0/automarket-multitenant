...
-- 8. BRANCHES TABLE (Sedes/Sucursales)
CREATE TABLE branches (
    ...
    status TEXT CHECK (status IN ('active', 'inactive', 'maintenance')) DEFAULT 'active',
    settings JSONB DEFAULT '{}',          -- Configuración de la sede
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(tenant_id, slug)
);

-- 9. LEADS TABLE (Sistema de Consultas)
CREATE TABLE leads (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- 10. FAVORITES TABLE (Favoritos de Compradores)
...
-- 11. MESSAGES TABLE (Sistema de Mensajería)
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS role TEXT CHECK (role IN ('corporate_admin', 'branch_manager', 'sales_person', 'buyer'));
...
ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS assigned_to UUID REFERENCES users(id);
...
CREATE POLICY "View branches from same tenant" ON branches FOR SELECT USING (
    tenant_id = get_current_tenant_id()
);

CREATE POLICY "Corporate admins can manage branches" ON branches 
    FOR ALL USING (
        tenant_id = get_current_tenant_id() AND
        EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'corporate_admin')
    );

-- Leads policies
ALTER TABLE leads ENABLE ROW LEVEL SECURITY;

CREATE POLICY "View leads from same tenant" ON leads FOR SELECT USING (
    tenant_id = get_current_tenant_id()
);

CREATE POLICY "Sales persons can manage assigned leads" ON leads 
    FOR ALL USING (
        tenant_id = get_current_tenant_id() AND
        (assigned_to = auth.uid() OR 
         EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('branch_manager', 'corporate_admin')))
    );
...
CREATE POLICY "Users can view messages from their leads" ON messages FOR SELECT USING (
    EXISTS (
        SELECT 1 FROM leads 
        WHERE leads.id = messages.lead_id 
        AND (leads.assigned_to = auth.uid() OR sender_id = auth.uid())
    )
);

CREATE POLICY "Users can send messages to their leads" ON messages FOR INSERT WITH CHECK (
